...
def create_link(
    ...
    if incoming_dim != outgoing_dim:
        assert outgoing_dim == 2 * incoming_dim, "%d != %d" % (
            outgoing_dim,
            2 * incoming,
        )
...
def _network_factory(weight_decay=1e-8):
    def factory_fn(image, reuse):
        with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=False):
            with slim.arg_scope(
                [slim.conv2d, slim.fully_connected, slim.batch_norm, slim.layer_norm],
                reuse=reuse,
            ):
...
def main():
    ...
    with tf.Session(graph=tf.Graph()) as session:
        ...
        image_var = tf.map_fn(
            lambda x: _preprocess(x), tf.cast(input_var, tf.float32), back_prop=False
        )
