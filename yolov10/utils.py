...
def draw_detections(
    ...
    for detection in detections:
        ...
        color = class_colors.get(class_name, (0, 255, 255))  # Default yellow
...
def validate_image_format(image: np.ndarray) -> bool:
    """
    Validate that image is in correct format for YOLOv10.
...
def get_model_memory_usage(model_info: Dict[str, Any]) -> str:
    """
    Get human-readable model memory usage information.
    ...
    return f"Parameters: {params}, FLOPs: {flops}"


def log_detection_summary(detections: List[Dict[str, Any]]) -> None:
    """
    ...
    logger.info(
        f"Found {len(detections)} objects (avg confidence: {avg_confidence:.3f})"
    )
...
def optimize_inference_settings(hardware_info: Dict[str, Any]) -> Dict[str, Any]:
    """
    Optimize inference settings based on hardware capabilities.
